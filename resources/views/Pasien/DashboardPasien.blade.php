...
<div class="main p-3">
    ...
    <dir class="board">
        ...
        @if (Auth::guard('pasien')->check())
            <p style="padding-left:25px;font-weight:bolder;color:red;font-size:15px;">Jangka waktu minum: {{ Auth::guard('pasien')->user()->mulai_minum }} - {{ Auth::guard('pasien')->user()->akhir_minum }} <br> di minum {{ Auth::guard('pasien')->user()-> medication_times}} kali sehari selama {{ Auth::guard('pasien')->user()-> time_to_take_medicine}} hari</p>
        @endif
        <table width="100%">
            <thead>
                <tr>
